...
st.set_page_config(page_title="Analisi VIP-positive/negative", layout="wide")
st.title("🔬 Classificazione e analisi cellule VIP (alpha7 e beta2)")

# 🔘 Selettore VIP-positive o VIP-negative
tipo_vip = st.radio("Seleziona il tipo di cellule da analizzare:", ["VIP-positive", "VIP-negative"], horizontal=True)

uploaded_file = st.file_uploader(f"Carica il file Excel unito ({tipo_vip}):", type=["xlsx"])
...
if uploaded_file:
    ...
    st.subheader("📈 Distribuzione puncta")
    col3, col4 = st.columns(2)
    with col3:
        if 'Children_alpha7_puncta_Count_alpha7' in df.columns:
            fig3, ax3 = plt.subplots()
            df['Children_alpha7_puncta_Count_alpha7'].hist(bins=30, ax=ax3)
            ax3.set_title("Distribuzione puncta Alpha7")
            st.pyplot(fig3)
        else:
            st.warning("Colonna 'Children_alpha7_puncta_Count_alpha7' non trovata.")

    with col4:
        if 'Children_beta2_puncta_Count_beta2' in df.columns:
            fig4, ax4 = plt.subplots()
            df['Children_beta2_puncta_Count_beta2'].hist(bins=30, ax=ax4)
            ax4.set_title("Distribuzione puncta Beta2")
            st.pyplot(fig4)
        else:
            st.warning("Colonna 'Children_beta2_puncta_Count_beta2' non trovata.")
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        ...
        pd.DataFrame({'Count': combo_counts, 'Percentuale': combo_perc}).to_excel(writer, sheet_name="Combinazioni")

    output.seek(0)
    st.download_button(
        label="📥 Scarica Excel con tutti i risultati",
        data=output,
        file_name=f"analisi_{tipo_vip.lower().replace('-', '_')}_completa.xlsx",
